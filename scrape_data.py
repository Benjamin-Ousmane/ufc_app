...
def parse_fighter_tott(soup):
    ...
    # parse first column of career statistics    
    tott = soup.find_all('ul', class_='b-list__box-list')[1]
    # loop through each tag to get text and next_sibling text
    for tag in tott.find_all('i'):
        # add text together and append to fighter_tott
        fighter_tott.append(tag.text + tag.next_sibling)
    
    # parse second column of career statistics    
    tott = soup.find_all('ul', class_='b-list__box-list')[2]
    # loop through each tag to get text and next_sibling text
    for tag in tott.find_all('i'):
        # add text together and append to fighter_tott
        fighter_tott.append(tag.text + tag.next_sibling)
...
def organise_fighter_tott(tott_from_soup, fighter_tott_column_names):
    '''
